  ...
  function handleEvent(event) {
    ...
    if (event.event === "startup_failed") {
      ...
      if (notificationNeeded(incomingTakesView && incoming && incoming.requestId===event.requestId))
        Quickshell.execDetached(["omarchy-notification-send","--app-name","Nearby","Incoming transfer",String(event.sender)+" wants to send "+Model.incomingSummary(event.files)])
    }
      ...
      if (notificationNeeded(!incomingTextPending))
        Quickshell.execDetached(["omarchy-notification-send","--app-name","Nearby","Text received","From "+String(event.sender || "")])
    }
